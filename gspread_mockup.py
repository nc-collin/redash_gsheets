...
def get_feed_df(url, api_key, orgname, cycle_id):
    ...
    pivot = pd.pivot_table(selection_df, values=['Rating'], index=['Reviewee ID'],
                           columns=['order'], aggfunc=np.mean)
    summary_df = feed_df[['Reviewee ID', 'Reviewee Name', 'Reviewee Email', 'Reviewer Name']]
    for i in range(1, 12):
...
def main():
    config = dotenv_values("mockup.env")
    ...
    service_acc_file = config['SERVICEACCOUNT']
    spreadsheet_id = config['SPREADSHEET_ID']

    print("Start Fetching Queries from Redash")
    user_data = get_user_df(url, api_key, orgname, now)
    print("User Data Fetched!")
    monthly_progress = get_progress_df(url, orgname, api_key, now, user_data)
    ...
    set_with_dataframe(worksheet2, user_data)
    ...
    worksheet4 = sh.worksheet("Feedback")
    sh.values_clear("Feedback!A2:C")
    set_with_dataframe(worksheet4, feedback_df)
    print("Feedback Sheet Updated")

    worksheet5 = sh.worksheet("Summary")
    sh.values_clear("Summary!A2:O")
    set_with_dataframe(worksheet5, summary_df)
